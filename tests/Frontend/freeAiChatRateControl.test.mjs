...
async function chatHarness(sendRequest, getMessages = async () => ({ data: { items: [], next_cursor: null } })) {
    ...
    const cleanup = [];
    const route = { params: { slug: "chat-writing", uuid: "conversation-a" }, meta: {}, name: "free-ai-model.chat" };
    const state = runInNewContext(
        script.replace(/^import .*;\r?\n/gm, "") + "\n({ sendMessage, conversation, selectedModel, loadingConversation, messageDraft, messages, sendingMessage, canSend, cooldownSeconds, cooldownUntil, cooldownClock, sendError });",
        {
            ref: (value) => ({ value }),
            ...
            getFreeAiCatalogSource: () => "general_chat",
            freeAiModelService: {
