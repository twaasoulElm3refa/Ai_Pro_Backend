...
const stateStorageKey = (uuid = activeConversation.value?.uuid || route.params.uuid || "draft") =>
    `chat5-tool-state:${activeSubToolId.value}:${uuid}`;
...
const ensureConversation = async () => {
    ...
    await router.replace(`/${homeService.getLang()}/subtool/${route.params.slug}/chat5/${conversation.uuid}`);
...
const startNewChat = async () => {
    ...
    try {
        ...
        await router.push(`/${homeService.getLang()}/subtool/${route.params.slug}/chat5/${conversation.uuid}`);
    } finally {
...
const deleteConversation = async (conversation) => {
    ...
    try {
        ...
        if (activeConversation.value?.uuid === conversation.uuid) {
            ...
            toolState.value = createDefaultStateForTool(activeSubToolId.value);
            removeResumeFile();
            await router.push(`/${homeService.getLang()}/subtool/${route.params.slug}/chat5`);
        }
    } finally {
        deletingUuid.value = "";
    }
