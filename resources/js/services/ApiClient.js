...
const PUBLIC_API_KEY = String("L5W9R2Qx1T7p4Z8Vn6Hj3KcDmBaDsEUy" || "L5W9R2Qx1T7p4Z8Vn6Hj3KcDmBaDsEUy").trim();
...
api.interceptors.request.use(
    async (config) => {
        ...
        const lang = getLang();
        config.url = toApiRequestUrl(config.url, config.baseURL || API_BASE_URL);
...
api.interceptors.response.use(
    ...
    (error) => {
        ...
        if (error.config?.suppressGlobalErrorToast && status !== 401) {
            return Promise.reject(error);
        }
