...
{
    public const SUB_TOOL_ID = 23;

    public const TOOL_KEY = 'ai_image_upscaler';

    private const ENDPOINT = 'tasks/image-upscaler';
    ...
    public static function supports(int $subToolId, ?string $toolKey = null, ?string $modelKey = null): bool
    {
        return $subToolId === self::SUB_TOOL_ID
            ...
            || strtolower(trim((string) $modelKey)) === 'image_upscaler';
    }
    ...
    public function handle(
        ...
        if ((int) $conversation->sub_tool_id !== self::SUB_TOOL_ID) {
            abort(422, 'هذه المحادثة ليست مخصصة لترقية جودة الصور.');
        }
        ...
        $lock = Cache::lock("image-upscaler:{$userId}:{$conversation->id}:{$idempotencyKey}", 360);
    ...
    private function handleLocked(
        ...
        try {
            ...
            Log::warning('Image upscaler failed.', [
                'user_id' => $userId,
                'conversation_id' => $conversation->id,
                'sub_tool_id' => self::SUB_TOOL_ID,
            ...
            $assistantMessage = Message::updateOrCreate(
                ...
                [
                    'conversation_id' => $conversation->id,
                    'role' => 'assistant',
                    'content' => 'تعذر ترقية جودة الصورة حاليًا. يرجى المحاولة مرة أخرى.',
                    'is_error' => true,
                    'metadata' => [
                        'success' => false,
                        'type' => 'error',
                        ...
                        'sub_tool_id' => self::SUB_TOOL_ID,
                        'conversation_uuid' => (string) $conversation->uuid,
                        'message' => 'تعذر ترقية جودة الصورة حاليًا. يرجى المحاولة مرة أخرى.',
                        'state' => $state,
                        'request_prompt' => $prompt,
                        'images' => [],
                        'files' => [],
    ...
    private function requestAi(
        ...
        if ($baseUrl === '' || $apiKey === '') {
            throw new RuntimeException('خدمة ترقية الصور غير مهيأة.');
        }
        ...
        if (! $response->successful() || ! is_array($result)) {
            throw new RuntimeException('خدمة ترقية الصور لم تُكمل الطلب.');
        }

        Log::debug('Image upscaler AI request completed.', [
    ...
    private function downloadFiles(array $remoteFiles, int $userId, string $conversationUuid): array
    {
        ...
        foreach (array_slice($remoteFiles, 0, 4) as $remoteFile) {
            ...
            try {
                ...
                Log::warning('Image upscaler file download failed.', [
                    'user_id' => $userId,
    ...
    private function downloadFile(array $remoteFile, int $userId, string $conversationUuid): array
    {
        ...
        $storagePath = "image-upscaler/{$userId}/{$conversationUuid}/{$publicId}.{$extension}";
        ...
        $displayFilename = Str::slug($baseName ?: 'image-upscaled').'-upscaled.'.$extension;
    ...
    private function publicFileData(array $file): array
    {
        ...
        return [
            ...
            'content_type' => $file['content_type'],
            'size_bytes' => $file['size_bytes'],
            'preview_url' => route('image-upscaler.files.preview', ['file' => $generatedFile]),
            'download_url' => route('image-upscaler.files.download', ['file' => $generatedFile]),
        ];
    }
