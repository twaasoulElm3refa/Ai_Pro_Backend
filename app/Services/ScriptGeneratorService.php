...
{
    public const SUB_TOOL_ID = 7;
    public const TOOL_KEY = 'ai_script_generator';
    public const MODEL_KEY = 'script_generator';
    public const ENDPOINT = 'tasks/script-generator/chat';

    private const MODEL_PROMPT = <<<'PROMPT'
You are a professional AI Script Generator.

Write a ready-to-use script that matches the requested platform, duration, language, tone, audience, and format. When the language is Arabic, write clear natural Arabic without mixing languages unless necessary. Match the requested duration closely.

When the format is "Hook + Body + CTA":
1. Start with a strong hook.
2. Explain the main idea clearly in the body.
3. End with a clear call to action.

When include_scene_notes is true, include useful scene, shot, or visual notes. When it is false, do not include scene notes. Apply extra_options when present. Avoid unnecessary markdown and return each final script in results[].text.
PROMPT;
    ...
    public function handle(
        ...
        Log::info('ScriptGenerator request received', [
            'user_id' => $userId,
        ...
        try {
            ...
            Log::error('ScriptGenerator failed', [
        ...
        if ($responseMessage === '') {
            $responseMessage = match ($type) {
                'question' => 'من فضلك أكمل البيانات المطلوبة لتوليد السكريبت.',
                'result' => 'تم توليد السكريبت بنجاح.',
                default => 'حدث خطأ أثناء توليد السكريبت.',
            };
        }
    ...
    public function normalizeState(mixed $state): array
    {
        ...
        $includeSceneNotes = $merged['include_scene_notes'] ?? null;
        if (is_bool($includeSceneNotes)) {
            $merged['include_scene_notes'] = $includeSceneNotes;
        } elseif (in_array($includeSceneNotes, [1, '1', 'true'], true)) {
            $merged['include_scene_notes'] = true;
        } elseif (in_array($includeSceneNotes, [0, '0', 'false'], true)) {
            $merged['include_scene_notes'] = false;
        } else {
            $merged['include_scene_notes'] = null;
        }

        $resultsCount = $merged['results_count'] ?? 2;
    ...
    private function inferStateFromContent(string $content, array $currentState): array
    {
        ...
        $isScriptRequest = $this->containsAny($text, [
            'سكريبت',
            'سيناريو',
            'script',
            'screenplay',
        ]);
        $isVideo = $this->containsAny($text, [
            'فيديو',
            'ريل',
            'يوتيوب',
            'تيك توك',
        ...
        if ($state['topic'] === null) {
            $state['topic'] = $this->extractTopic($content);
        }

        if ($state['script_type'] === null && ($isScriptRequest || $isVideo)) {
            $state['script_type'] = $this->containsAny($text, ['إعلان', 'اعلان', 'advertisement', 'commercial'])
                ? 'Ad Script'
                : ($this->containsAny($text, ['بودكاست', 'podcast']) ? 'Podcast Script' : 'Video Script');
        }

        if ($state['platform'] === null) {
            $state['platform'] = match (true) {
                $this->containsAny($text, ['تيك توك', 'tiktok']) => 'TikTok',
                $this->containsAny($text, ['يوتيوب', 'youtube']) => 'YouTube',
                $this->containsAny($text, ['انستجرام', 'إنستغرام', 'instagram', 'reel', 'ريل']) => 'Instagram Reels',
                $this->containsAny($text, ['بودكاست', 'podcast']) => 'Podcast',
                $isVideo => 'General Video',
                default => null,
            };
        }

        if ($state['language'] === null && ($isScriptRequest || $isVideo)) {
            $state['language'] = $isArabic ? 'Arabic' : 'English';
        }

        if ($state['tone'] === null && ($isScriptRequest || $isVideo)) {
            $state['tone'] = match (true) {
                $this->containsAny($text, ['رسمي', 'formal']) => 'Formal',
                $this->containsAny($text, ['احترافي', 'professional']) => 'Professional',
                $this->containsAny($text, ['مرح', 'funny', 'humorous']) => 'Humorous',
                $this->containsAny($text, ['مقنع', 'persuasive']) => 'Persuasive',
                default => 'Engaging',
            };
        }

        if ($state['audience'] === null && ($isScriptRequest || $isVideo)) {
            $state['audience'] = match (true) {
                $this->containsAny($text, ['أطفال', 'اطفال', 'children', 'kids']) => 'Children',
                $this->containsAny($text, ['مسوق', 'marketer']) => 'Marketers',
                $this->containsAny($text, ['أصحاب الأعمال', 'رواد الأعمال', 'business owners', 'entrepreneurs']) => 'Business Owners',
                default => 'General Audience',
            };
        }

        if ($state['duration'] === null) {
            $state['duration'] = $this->extractDuration($content);
        }

        if ($state['format'] === null && ($isScriptRequest || $isVideo)) {
            $state['format'] = 'Hook + Body + CTA';
        }

        if ($state['include_scene_notes'] === null && ($isScriptRequest || $isVideo)) {
            $state['include_scene_notes'] = $this->containsAny($text, [
                'ملاحظات مشاهد',
                'ملاحظات للمشاهد',
                'لقطات',
                'scene notes',
        ...
        if (count($state['extra_options']) === 0 && ($isScriptRequest || $isVideo)) {
            $state['extra_options'] = ['Strong opening hook', 'Clear ending'];
        }
    ...
    private function extractDuration(string $content): ?string
    {
        if (preg_match('/(\d+)\s*(ثانية|ثواني|second|seconds)\b/iu', $content, $matches) === 1) {
            return ((int) $matches[1]).' seconds';
        }

        if (preg_match('/(\d+)\s*(دقيقة|دقائق|minute|minutes)\b/iu', $content, $matches) === 1) {
            return ((int) $matches[1]).' minutes';
        }
